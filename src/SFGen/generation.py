...
class SecurityLogitsProcessor(LogitsProcessor):
    def __init__(self, threshold=0.0, top_k=0, prob_max=1.0, prob_min=0.0):
        if not isinstance(threshold, float):
            raise ValueError(f"`threshold` has to be a float, but is {threshold}")

        if not isinstance(top_k, int) or top_k < 0:
            raise ValueError(f"`top_k` has to be a positive int, but is {top_k}")
    ...
    def __call__(
        ...
        if_scores_raw_logits: bool = True,
    ) -> torch.FloatTensor:
        """
        Args:
            scores [bsz * beam_size, vocab_size]: 
            security_scores [bsz * beam_size, vocab_size]: 
        """
        if security_scores is None:
            return scores

        # print('avaliable tokens: ', torch.sum(~torch.isinf(scores), dim=1))
        raw_lm_probs = nn.functional.softmax(scores, dim=1) if if_scores_raw_logits else torch.exp(scores)        
        if self.top_k > 0:
            topk_probs, topk_indices = torch.topk(raw_lm_probs, self.top_k, dim=1)
            topk_probs_renormalized = topk_probs / topk_probs.sum(dim=1, keepdim=True)

            lm_probs = torch.zeros_like(raw_lm_probs)
            lm_probs.scatter_(1, topk_indices, topk_probs_renormalized)
        else:
            lm_probs = raw_lm_probs

        # max_lm_probs = torch.max(lm_probs, dim=1, keepdim=True)[0] - 1e-7
        # threshold_ = torch.zeros_like(max_lm_probs) + self.threshold
        # threshold_[threshold_ > max_lm_probs] = max_lm_probs[threshold_ > max_lm_probs]

        # print("max_lm_probs: ", max_lm_probs)
        # print("threshold_: ", threshold_)

        # security_scores: [batch_size, vocab_size]
        security_scores = torch.clamp(security_scores, max=0.0, min=-1.0)
        secure_probs = (1 + security_scores - self.threshold) / (1 - self.threshold)
        # secure_probs = (1 + security_scores - threshold_) / (1 - threshold_)
        secure_probs = torch.clamp(secure_probs, max=1.0, min=self.prob_min)

        lm_probs = lm_probs * secure_probs
        lm_probs = lm_probs / lm_probs.sum(dim=1, keepdim=True)

        for _ in range(1):
            mask = lm_probs > secure_probs
            # print("lm_probs: ", lm_probs[:10])
            # print("secure_probs: ", secure_probs[:10])
...
class CustomizedGenerationMixin:
    """
    ...
    def __init__(self, model, security = None) -> None:
        self.model = model
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        self, model, inputs_tensor: torch.Tensor, model_kwargs, model_input_name: Optional[str] = None
    ) -> Dict[str, Any]:
        # 1. get encoder
    ...
    def _reorder_security_cache(self, past, beam_idx):
        return self.security._reorder_cache(past, beam_idx)
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        ...
        if self.config.is_encoder_decoder:
            base_model = getattr(self.model, self.model.base_model_prefix, None)
    ...
    def generate(
        ...
        negative_prompt_ids: Optional[torch.Tensor] = None,
        negative_prompt_attention_mask: Optional[torch.Tensor] = None,
	    secure_threshold: Optional[float] = 0.0,
        secure_top_k: Optional[int] = 0,
        secure_prob_min: Optional[float] = 0.0,
        secure_prob_max: Optional[float] = 1.0,
        **kwargs,
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            ...
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(
                self.model, inputs_tensor, model_kwargs, model_input_name,
            )

            raise Exception("Not implemented for encoder-decoder language model")
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            ...
            if self.security:
                logits_warper = LogitsProcessorList([
                    SecurityLogitsProcessor(
                        threshold=secure_threshold,
                        top_k=secure_top_k,
                        prob_min=secure_prob_min,
                        prob_max=secure_prob_max,
                    )
                ])
            else:
                logits_warper = None
            ...
            if self.security:
                logits_warper.append(SecurityLogitsProcessor(
                    threshold=secure_threshold,
                    top_k=secure_top_k,
                    prob_min=secure_prob_min,
                    prob_max=secure_prob_max,
            ...
            input_ids_keep = deepcopy(input_ids) 
            input_ids, model_kwargs = self._expand_inputs_for_generation(
                input_ids=input_ids,
            ...
            if self.security:
                _, security_kwargs = self._expand_inputs_for_generation(
                ...
                model_kwargs["security_encoder_outputs"] = security_kwargs["encoder_outputs"]
                logits_warper = LogitsProcessorList([
                    SecurityLogitsProcessor(
                        threshold=secure_threshold,
                        top_k=secure_top_k,
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        this_peer_finished = False  # used by synced_gpus only
        security_kwargs = deepcopy(model_kwargs)
        if self.config.is_encoder_decoder and self.security:
            security_kwargs["encoder_outputs"] = model_kwargs["security_encoder_outputs"]
        ...
        while True:
            ...
            if self.security:
                security_inputs = self.security.prepare_inputs_for_generation(input_ids, **security_kwargs)
                security_outputs = self.security(
                    **security_inputs
                )
                Q_values = security_outputs.logits[:, -1, :]
            else:
                Q_values = None
            ...
            next_tokens_scores = logits_warper(
                input_ids, next_tokens_scores, security_scores=Q_values, if_scores_raw_logits=True)
            ...
            if self.security:
                security_kwargs = self._update_model_kwargs_for_generation(
                    security_outputs, security_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
                )
        ...
        **model_kwargs,
        ...
        while True:
            ...
            next_token_scores = logits_warper(
                input_ids, next_token_scores, security_scores=Q_values, if_scores_raw_logits=True)
            ...
            # sample
            if type(logits_warper[-1]) == SecurityLogitsProcessor:
        ...
        while True:
            ...
            if self.security:
                next_token_scores_processed = logits_warper(
                    ...
                    security_scores=Q_values,
                    if_scores_raw_logits=False
                )
                next_token_scores_processed = torch.log(next_token_scores_processed)
            ...
            if self.security:
                ...
                if security_kwargs["past"] is not None:
                    security_kwargs["past"] = self._reorder_security_cache(security_kwargs["past"], beam_idx)
